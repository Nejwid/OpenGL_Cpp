...
float pojazd_x1 = 0.05f;
float pojazd_x2 = -0.05f;
float pojazd_y1 = -0.9f;
float pojazd_y2 = -1.f;
float pojazd_z1 = 2.05f;
float pojazd_z2 = 1.95f;
...
{
	if (pojazd_x2 > -0.9f && glfwGetKey(window, GLFW_KEY_LEFT) != GLFW_PRESS) {
		if (glfwGetKey(window, GLFW_KEY_RIGHT) == GLFW_PRESS)
		{
			pojazd_x1 = pojazd_x1 - speed;
			pojazd_x2 = pojazd_x2 - speed;
		}
	}

	if (pojazd_x1 < 0.9f && glfwGetKey(window, GLFW_KEY_RIGHT) != GLFW_PRESS)
	{
